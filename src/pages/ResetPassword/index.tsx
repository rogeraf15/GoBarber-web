...
const Signin: React.FC = () => {
  ...
  const handleSubmit = useCallback(async (data: ResetFormDate) => {
    try {
      formRef.current?.setErrors({});

      const schema = Yup.object().shape({
        password: Yup.string().required('Senha obrigatória'),
        password_confirmation: Yup.string().oneOf([Yup.ref('password')], 'Confiramação incorreta'),
      });
